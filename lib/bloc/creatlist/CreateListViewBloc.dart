...
class CreateListViewBloc extends ChangeNotifier {
 ...
 CreateListMod createLis =   filteredOrders.firstWhere((order) => order.id == id);

        return createLis.lines.map((line) {
      return '${line.barcode} (Qty: ${line.qty})'; // Format: "barcode (Qty: quantity)"
    }).toList();
  }
 List<String> getAllBarcodes2(String id) {
   CreateListMod createLis =   filteredOrders.firstWhere((order) => order.id == id);
